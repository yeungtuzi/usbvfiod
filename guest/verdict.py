...
2. the md5 of the copied file equals the expected digest;
3. no USB enumeration after the guest uptime at which the migration happened;
4. no reset / timeout / I/O error after that uptime.

The guest uptime at the migration is obtained from the heartbeat lines, which
carry both the wall-clock epoch and /proc/uptime, so no host/guest clock
assumption is needed.

Usage:
  verdict.py --guest-log LOG --expected-md5 MD5 --migration-epoch EPOCH

Exit status: 0 if all criteria pass, 1 otherwise.
"""
...
ENUM = re.compile(r"new (?:high|full|super)-speed USB device")
BAD = [
    ("reset", re.compile(r"usb [0-9-]+: reset")),
...
def main() -> int:
    ...
    print(f"migration epoch      : {args.migration_epoch:.3f}")
    ...
    if beats:
        ...
        if mig_uptime is None:
            # outside the heartbeat range: extrapolate from the nearest pair
    ...
    print(f"guest uptime @migr.  : {mig_uptime:.2f} s" if mig_uptime is not None
          else "guest uptime @migr.  : <unavailable>")

    # --- kernel events after the migration, from the guest's own dmesg -------
    dmesg = []
    in_dmesg = False
    ...
    if not dmesg:
        print("guest dmesg section  : MISSING (cannot verify enumeration/reset)")
        late_enum = late_bad = -1
    else:
        late_enum = 0
        late_bad = 0
        seen_enum = []
        ...
        for line in dmesg:
            m = KTS.match(line)
            if not m or mig_uptime is None:
                continue
            ts = float(m.group(1))
            if ts <= mig_uptime:
                continue
    ...
    ok = spans and md5_ok and finished and late_enum == 0 and late_bad == 0
