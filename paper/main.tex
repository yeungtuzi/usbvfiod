...
\emph{How much does \PassCount{} out of \TotalRuns{} establish?} Not as much as the number
suggests, and we prefer to say so. Ten consecutive successes give a one-sided 95\,\% lower confidence bound of $\PassLowerOneSided$ on the pass probability, i.e.\ the one-sided 95\,\% upper bound on the residual failure rate is $\FailUpper$; the two-sided 95\,\% Clopper--Pearson interval is $[\CILow, \CIHigh]$. Against the pre-fix behaviour we observed---one failure in
...
The downtime figures are also less precise than their range suggests: the median is \DowntimeMedian\,ms but the order-statistic 95\,\% confidence interval for it spans essentially the whole observed range (\DowntimeMin--\DowntimeMax\,ms). The distribution is
...
rather than to independent noise. Copy duration varied between \CopyMin\,s and \CopyMax\,s; a no-migration control run is reported below, so we do not attribute that variation to the migration.
