...
\ExposureRuns{} of the \ExposureMeasured{} runs analysed had at least one
...
so without the kick the guest is never told to look at the ring. An exposure
rate of \ExposureRate\,\% is consistent with the pre-fix failure rate we
observed (one run in five), and it explains why the defect is intermittent: the
window is only a few milliseconds wide while completions arrive at a few hundred
per second.
