...
while the first client is still connected. We identify and fix two further defects that
...
hand-over window---a reset-capability parsing bug in the vfio-user
crate, and we describe a measurement harness that derives the verdict from the
...
answer, and it is ''not'' what this paper does: it transfers device state, which
requires both ends to run a compatible server and does not preserve a host-side
session that is not expressible as state. Our problem is the same-host case,
...
external transport. QEMU's USB emulation and redirection \cite{qemuusb} also provide
USB to a guest, but physical \code{usb-host} devices are normally refused for
migration, and redirection reconnects the client transport rather than
preserving a host-side session.
...
QEMU \code{usb-host} (physical passthrough) \cite{qemuusb} & device bound inside the VMM process & migration is normally refused by the VMM \\
USB redirection (\code{usbredir}) \cite{qemuusb} & transport reconnects to the client & client-side reconnection semantics differ \\
Kernel VFIO migration (non-USB) \cite{kernelvfio} & device-defined migration state & requires device support; not applicable to USB \\
\bottomrule
\end{tabular}
\end{table}
...
Linux 6.17.4 (Proxmox kernel), 8 cores and 62\,GiB of RAM, which runs KVM. The
...
server. A protocol-level migration mechanism is being proposed for
vfio-user \cite{komatsu2026vfiousermig}, but the implementation used here
...
Clopper--Pearson 95\,\% confidence interval for the pass probability of
$[0.74, 1.0]$, i.e.\ the data are consistent with a residual failure rate of up
to roughly 26\,\%. Against the pre-fix behaviour we observed---one failure in
...
Fig.~\ref{fig:copy} shows the guest's copy progress for a demonstration run
(not one of the ten acceptance runs) together with the migration instant. The copy is visibly interrupted in the sense that the
...
\label{sec:rate}
Fig.~\ref{fig:rate} compares the USB packet rate across the hand-over
...
transfers stop at the hand-over and do not resume within the observed 39\,s
window; with the fix they continue until the copy completes.}
\label{fig:rate}
\end{figure}
...
\emph{Statistical.} $n=10$, no confidence intervals in the headline numbers,
and a pre-fix rate estimated from $n=5$; the statistical power is discussed in
