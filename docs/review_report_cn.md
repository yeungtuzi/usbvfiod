...
### 12.8 轮次 9 追加之四：A5 真机注入与重复批（2026-09-25）

| 轴 | 内容 | 结论 |
|---|---|---|
| 系统 | A5 在**真实摘除**下生效（`usb-a5`）：设备在目标端仅暂存时被 `remote --detach 1 7` 拔掉，`commit` 被拒并给出 `EPREFLIGHT_A5_DEVICE_GONE`；整跑只装过一次线（源端启动那次）、源端线全程未动，随后由 VETO 终止目标端，迁移失败而源端继续运行 | **Accept** |
| 方法学 | 该跑同时回归了旧二进制控制协议（`--detach`）在多客户端模式下的路径；另有一组独立的 10 次默认配置批（`usb-batch-binding`，标签与实际配置不一致已如实记录）10/10 通过，使交付代码的默认配置累计 **20/20**，与既有 20/20 判定标准一致 | **Accept** |
| 写作与工件 | 论文等价性段落加入重复臂宏（`\FinalRepRuns/\FinalRepPass`）；开发日志 D32/D33 如实记录标签与配置不一致；原始日志归档 27 份 + 校验（目标端 `sha256sum -c` 全 OK） | **Accept** |

**仍存在的边界（不影响上述结论，已在文档中标注）**：
B1（候选 region 读取完整性）未实现（A3 的 DMA 覆盖间接拦住"什么都没做"的候选）；
binding 模式尚无批次级证据（有单次 T1、否决、A5 真机与 4 个专项测试）；
"提交后崩溃保源"在本 VMM 上不可达（`usb-b5` 负面结果，主机制改为提交前否决）；
论文三评审面板未针对 M6/binding 改动重跑。
